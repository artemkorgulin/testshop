...
class Application  //класс маршрутизатор, подбирает нужный контролер для обработки данных
{
	...
	public function Run()// запуск процесса обработки данных
	{
		...
		$controller = "Shop\\Application\\Controllers\\" . $name_contr;

		try {
			$contr = new $controller; //создаем экземпляр класса контролера
		} catch (\Error $e) {
			$errorClass = "Shop\\Application\\Controllers\\Pageerror";
			$contr = new $errorClass;
		}
